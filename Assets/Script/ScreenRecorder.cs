...
public class ScreenRecorder : MonoBehaviour
{
	...
	private bool terminateThreadWhenDone = false;

	public void startRecord(string folderName)
	{
		// Prepare the data directory
		savePath = persistentDataPath + "/" + folderName;

		print("Capturing to: " + savePath + "/");
	...
	void Start()
	{
		...
		savePath = persistentDataPath +  "/Default";
	...
	private void EncodeAndSave()
	{
		...
		while (threadIsProcessing)
		{
			...
			{
				// Generate file path
				string path = savePath + "/frame" + savingFrameNumber + ".bmp";

				// Dequeue the frame, encode it as a bitmap, and write it to the file
				using (FileStream fileStream = new FileStream(path, FileMode.Create))
				{
					writeBitmap(fileStream, screenWidth, screenHeight, frameQueue.Dequeue());
					fileStream.Close();
				}
	...
	private static void writeBitmap(Stream stream, int width, int height, byte[] imageData)
	{
